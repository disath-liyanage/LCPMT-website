...
export async function createProject(formData: FormData) {
  ...
  const location = location_type === 'Online' ? 'Online' : formData.get('location') as string
...
export async function updateProject(id: string, formData: FormData) {
  ...
  const { error: dbError } = await supabase.from('projects').update({
    title, description, date, location_type, location, avenue, collaborators, images: finalImages, main_image: finalMainImage
  }).eq('id', id)
...
export async function getProjects() {
  ...
  if (error) return []
  return data.map(project => ({ ...project, category: project.avenue, main_image: project.main_image || (project.images && project.images[0]) || project.image_url || null, images: project.images || [] }))
}

export async function getFeaturedProjects() {
  const supabase = await createClient()
  const { data, error } = await supabase.from('projects').select('*').eq('featured_on_main', true).order('date', { ascending: false }) 
  if (error) return []
  return data.map(project => ({ ...project, category: project.avenue, main_image: project.main_image || (project.images && project.images[0]) || project.image_url || null, images: project.images || [] }))
}
...
export async function toggleFeatureProject(id: string, currentStatus: boolean) {
  const supabase = await createClient()
  await supabase.from('projects').update({ featured_on_main: !currentStatus }).eq('id', id)
  revalidatePath('/admin/projects')
  revalidatePath('/')
}
...
export async function deleteProject(id: string) {
  const supabase = await createClient()
  const { data: project } = await supabase.from('projects').select('images, image_url').eq('id', id).single()
  const allImagesToDelete = [...(project?.images || [])];
  if (project?.image_url) allImagesToDelete.push(project.image_url); 
  if (allImagesToDelete.length > 0) {
    const imagePaths = allImagesToDelete.map((url: string) => url.split(`/public/${BUCKET_NAME}/`)[1]).filter(Boolean)
    if (imagePaths.length > 0) await supabase.storage.from(BUCKET_NAME).remove(imagePaths)
  }
